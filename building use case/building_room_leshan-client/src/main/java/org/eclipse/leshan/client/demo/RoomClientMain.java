...
public class RoomClientMain {
	...
	private static final Logger LOG = LoggerFactory.getLogger(RoomClientMain.class);

	private final static String[] modelPaths = new String[] { "3306.xml", "3300.xml" };

	private static final int OBJECT_ID_ACTUATION = 3306;
	private static final int OBJECT_ID_GENERIC_SENSOR = 3300;

	private final static String DEFAULT_ENDPOINT = "RoomClient";
	private final static String USAGE = "java -jar leshan-client-demo.jar [OPTION]";

	private static SprinklerHandler _sprinklerHandler;
	private static FireSensorHandler _fireSensorHandler;
	...
	public static void createAndStartClient(String endpoint, String localAddress, int localPort,
		...
		_sprinklerHandler = new SprinklerHandler();
		_fireSensorHandler = new FireSensorHandler();
		// Initialize model
		...
		initializer.setInstancesForObject(OBJECT_ID_GENERIC_SENSOR, _fireSensorHandler);
		initializer.setInstancesForObject(OBJECT_ID_ACTUATION, _sprinklerHandler);
		List<LwM2mObjectEnabler> enablers = initializer.create(SECURITY, SERVER, OBJECT_ID_ACTUATION,
				OBJECT_ID_GENERIC_SENSOR);
		...
		try (Scanner scanner = new Scanner(System.in)) {
			while (scanner.hasNext()) {
				...
				if (ip.equals("f") || ip.equals("F")) {
					_fireSensorHandler.toggleFireSensorState();
				} else if (ip.equals("s") || ip.equals("S")) {
					_sprinklerHandler.toggleSprinklerState();
				}
			}
		}
	}

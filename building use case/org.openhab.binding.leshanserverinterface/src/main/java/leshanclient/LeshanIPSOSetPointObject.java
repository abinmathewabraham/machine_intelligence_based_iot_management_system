...
public class LeshanIPSOSetPointObject implements ILeshanClientObjectHandler, ICallbacksForServerResponse {

    private LeshanClientObjectInfoVO _ipsoSetPointVO;
    ...
    public LeshanIPSOSetPointObject(ICallbacksToThingHandler thingHandler) {
    ...
    public void setLeshanClientObjectInfo(String name, String regId, int typeId, int instanceId) {
        _ipsoSetPointVO = new LeshanClientObjectInfoVO(name, regId, typeId, instanceId);
    }
    ...
    public void handleChannelCommands(ChannelUID channelUID, Command command) {
        if (channelUID == _readApplicationTypeChannelUID) {
            if (command == OnOffType.ON) {
                ...
                ServerCommandsHandler.getInstance().sendCommands(LeshanServerCommands.READ_RESOURCE, _ipsoSetPointVO,
                        this, extraParams);
            }
        } else if (channelUID == _readValueChannelUID) {
            if (command == OnOffType.ON) {
                ArrayList<Object> extraParams = new ArrayList<>();
                extraParams.add("5900");
                ServerCommandsHandler.getInstance().sendCommands(LeshanServerCommands.READ_RESOURCE, _ipsoSetPointVO,
                        this, extraParams);
            }
        }
                ...
                switch (responseObj.resourceId) {
                    ...
                    case 5900:
                        _thingHandler.callbackForUpdatingChannel(_readValueResponseChannelUID,
                                new StringType(responseObj.getContentValue().equals("1.0") ? "RoomB" : "RoomA"));
    ...
    private String getClientPrefix(String label) {
        return _ipsoSetPointVO.name + ":IPSOSetPoint:" + _ipsoSetPointVO.instanceId + ":" + label;
    }

    private String getClientPrefix(String label, String seperator) {
        return _ipsoSetPointVO.name + seperator + "IPSOSetPoint" + seperator + _ipsoSetPointVO.instanceId + seperator
                + label;
    }

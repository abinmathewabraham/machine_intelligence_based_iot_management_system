...
public class BuildingServerClientMain {

	private static final Logger LOG = LoggerFactory.getLogger(BuildingServerClientMain.class);

	private final static String[] modelPaths = new String[] { "3306.xml", "3308.xml" };

	private static final int OBJECT_ID_ACTUATION = 3306;
	private static final int OBJECT_ID_SETPOINT = 3308;

	private final static String DEFAULT_ENDPOINT = "BuildingServerClient";
	private final static String USAGE = "java -jar leshan-client-demo.jar [OPTION]";

	private static ServerActuationHandler _serverActuationHandler;
	private static ServerLocationObject _serverLocationHandler;
	...
	public static void createAndStartClient(String endpoint, String localAddress, int localPort,
		...
		_serverActuationHandler = new ServerActuationHandler();
		_serverLocationHandler = new ServerLocationObject();
		// Initialize model
		...
		initializer.setInstancesForObject(OBJECT_ID_SETPOINT, _serverLocationHandler);
		initializer.setInstancesForObject(OBJECT_ID_ACTUATION, _serverActuationHandler);
		List<LwM2mObjectEnabler> enablers = initializer.create(SECURITY, SERVER, OBJECT_ID_ACTUATION,
				OBJECT_ID_SETPOINT);
		...
		// Change the States through the Console
		try (Scanner scanner = new Scanner(System.in)) {
			while (scanner.hasNext()) {
				String ip = scanner.next();
				if (ip.equals("r") || ip.equals("R")) {
					_serverLocationHandler.changeRoom();
				} else if (ip.equals("s") || ip.equals("S")) {
					_serverActuationHandler.toggleServerActuationState();
				}
			}
